...
Instance Reader::readInstanceFromArgs(const vector<string>& input_vector) {
    if (input_vector.size() % 2 != 0 || input_vector.size() < 8) { // assumption: no empty instances, i.e. at least 1 item
        cerr << "Not enough command params given, have to be at least 8 (no empty instances!)\n";
        printUsageAndExit();
    }
    //creating Instance from commandline arguments
    int numberOfItems = (input_vector.size()-6)/2;
    Instance instance = Instance(numberOfItems);
    instance.setCapacity(stoi(input_vector[5]));
    int ctr = 0;
    for (int i = 6; i < input_vector.size(); i+=2) {
...
void Reader::printUsageAndExit() {
    ...
    cerr << "    dat timelimit iterationlimit starttemperature factor knapsackfile_location OR\n";
    cerr << "    param timelimit iterationlimit starttemperature factor W c1 w1 c2 w2 ... cn wn\n";
    exit(EXIT_FAILURE);
}